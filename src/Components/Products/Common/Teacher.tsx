...
const Teacher = ({ error = false, message = "", value, onChange }: Props) => {
    //Static
    const id = "teachers";
    const placeholder = "Select teacher";
    ...
    //Apollo
    const { data, loading, fetchMore, refetch, variables } = useQuery(TEACHER_LIST, { variables: { searchDto: defaultSearch } });
    ...
    const onFetchMore = () => {
        fetchMore({
            ...
            updateQuery: (previousData, { fetchMoreResult }) => {
                ...
                const updatedFeed = [
                    ...(previousData.getTeachers.results || []),
                    ...(fetchMoreResult.getTeachers.results || []),
                ];

                return {
                    ...previousData,
                    getCategories: {
                        ...previousData.getTeachers,
                        results: updatedFeed,
                        meta: fetchMoreResult.getTeachers.meta,
                    },
                };
            },
        })
    ...
    //Effect
    useEffect(() => {
        const options = data?.getTeachers.results?.map(a => {
            ...
            <div className="relative" ref={containerRef}>
                ...
                <div id="teacher_scroll_target" className={`max-h-[400px] absolute top-full mt-2 left-0 w-full border border-solid transition-all duration-200 ease-[cubic-bezier(0.4, 0, 0.2, 1)] border-gray-200 rounded-md z-40 bg-white shadow-3xl overflow-auto origin-center ${open ? "opacity-100 scale-100 visible" : "opacity-0 invisible scale-95"}`}>
                    ...
                    <InfiniteScroll
                        ...
                        next={() => onFetchMore()}
                        hasMore={!variables?.searchDto.search ? Number(data?.getTeachers.meta?.currentPage) < Number(data?.getTeachers.meta?.totalPages) : false}
                        loader={<span></span>}
                        scrollableTarget="teacher_scroll_target"
                    >
                        <ul ref={ref}>
                            {list.map((item, i) => (
                                <li
