...
import { RUNNING_SALES_LIST } from "@/Apollo/query/sales/sales";
...
const Flash = ({ error = false, message = "", value, onChange }: Props) => {
    //Static
    const id = "sales";
    const placeholder = "Select sale";
    ...
    //Apollo
    const { data, loading, fetchMore, refetch, variables } = useQuery(RUNNING_SALES_LIST, { variables: { searchDto: defaultSearch } });
    ...
    const onFetchMore = () => {
        fetchMore({
            ...
            updateQuery: (previousData, { fetchMoreResult }) => {
                ...
                const updatedFeed = [
                    ...(previousData.getRunningSales.results || []),
                    ...(fetchMoreResult.getRunningSales.results || []),
                ];

                return {
                    ...previousData,
                    getCategories: {
                        ...previousData.getRunningSales,
                        results: updatedFeed,
                        meta: fetchMoreResult.getRunningSales.meta,
                    },
                };
            },
        })
    ...
    //Effect
    useEffect(() => {
        const options = data?.getRunningSales.results?.map(a => {
            return { value: a.id?.toString(), label: a.title }
        });
    ...
    return (
        <div className={`${error ? "mb-2" : "mb-0"}`}>
            ...
            <div className="relative" ref={containerRef}>
                ...
                <div id="flash_scroll_target" className={`max-h-[400px] absolute top-full mt-2 left-0 w-full border border-solid transition-all duration-200 ease-[cubic-bezier(0.4, 0, 0.2, 1)] border-gray-200 rounded-md z-40 bg-white shadow-3xl overflow-auto origin-center ${open ? "opacity-100 scale-100 visible" : "opacity-0 invisible scale-95"}`}>
                    ...
                    <InfiniteScroll
                        ...
                        next={() => onFetchMore()}
                        hasMore={!variables?.searchDto.search ? Number(data?.getRunningSales.meta?.currentPage) < Number(data?.getRunningSales.meta?.totalPages) : false}
                        loader={<span></span>}
                        scrollableTarget="flash_scroll_target"
                    >
                        <ul ref={ref}>
                            {list.map((item, i) => (
                                <li
